...
### Tasks Completed
- [x] Created `sdaa/src/tools/context_ops.py` with `ContextManager`, `reset_context`, `append_to_notebook`, and `read_notebook`.
- [x] Updated `sdaa/src/agents/workers.py` to inject new tools and instructions.
- [x] Updated `main.py` to initialize context management with `SharedMemorySessionService`.
- [x] Verified context clearing and notebook functionality via tests.

### Next Steps & Continuity
- **Refactor Prompts**: The notebook instructions are currently appended in code (`sdaa/src/agents/workers.py`) for immediate testing. For production, remove this code appending and move the instructions directly into the Langfuse prompt templates (`permissions-agent`, `negative-constraints-agent`, `security-boundaries-agent`).
- Run a full audit to verify the strategy works under load.
